  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24">
      <header className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-20 border-b border-slate-200">
        <div className="max-w-md mx-auto px-4 py-3 flex justify-between items-center">
          ...
          <div className="relative" ref={menuRef}>
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="w-9 h-9 rounded-full bg-blue-100 border border-blue-200 flex items-center justify-center text-blue-700 font-bold text-sm shadow-sm active:scale-90 transition-transform">
      ...
      <main className="max-w-md mx-auto px-4 py-6">
        <div className="space-y-6">
          <section className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100">
            <h2 className="text-sm font-bold text-slate-400 uppercase mb-4 px-1">家族情報</h2>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl">
                <span className="text-sm font-bold text-slate-500">家族ID</span>
                <span className="text-sm font-extrabold text-blue-600">{familyId}</span>
              </div>
              <div className="space-y-2">
                <p className="text-[10px] font-bold text-slate-400 ml-1 uppercase tracking-wider">家族メンバー</p>
                <div className="space-y-2">
                  {familyMembers.map((member) => (
                    <div key={member.name} className="group flex items-center justify-between p-3 bg-white border border-slate-100 rounded-2xl shadow-sm hover:border-blue-100 transition-colors">
                        ...
                        <div>
                          ...
                          {editingMemberName === member.name ? (
                            <div className="flex items-center gap-1 mt-0.5">
                              <span className="text-[10px] font-bold text-slate-400">サイズ:</span>
                              <input
                                type="text"
                                value={tempSize}
                                onChange={(e) => setTempSize(e.target.value)}
                                className="text-[10px] font-bold text-blue-600 border-b border-blue-400 focus:outline-none w-12 bg-transparent p-0"
                                autoFocus
                              />
                            </div>
                          ) : (
                            <p className="text-[10px] font-bold text-slate-400 mt-0.5">サイズ: {member.defaultSize}</p>
                          )}
                        </div>
                      </div>
                      <button 
              ...
              <div className="space-y-2 pt-4 border-t border-slate-100 mt-4">
                <p className="text-[10px] font-bold text-slate-400 ml-1 uppercase tracking-wider">新しいメンバーを追加</p>
                <div className="space-y-3">
                  <input
                    ...
                    placeholder="名前（例: パパ）"
                    className="w-full px-4 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-sm"
                  />
                  <input
                    type="text"
                    value={newMemberSize}
                  ...
                  />
                  <button
                    className="w-full py-2.5 bg-blue-600 text-white rounded-xl font-bold text-sm shadow-md shadow-blue-500/20 active:scale-95 transition-transform"
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-slate-200 flex justify-around pt-3 pb-6 px-2 z-30">
        ...
        <button onClick={() => router.push('/')} className="flex flex-col items-center flex-1 text-slate-400">
          <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7"></rect><rect x="14" y="3" width="7" height="7"></rect><rect x="14" y="14" width="7" height="7"></rect><rect x="3" y="14" width="7" height="7"></rect></svg>
          <span className="text-[10px] mt-1 font-medium">一覧</span>
        </button>
        <button onClick={() => router.push('/register')} className="flex flex-col items-center flex-1 text-slate-400">
